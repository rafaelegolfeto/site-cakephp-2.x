				...
				<tbody>
					<?php foreach ($news as $news) : ?>
						<tr class="odd gradeX">
							...
							<td class="center"><?php echo $news['Category']['name'] ?></td>
							<td><?php echo h($news['News']['created']); ?>&nbsp;</td>
							<td><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $news['News']['id'])); ?>
								<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $news['News']['id']), array('confirm' => __('Are you sure you want to delete # %s?', $news['News']['id']))); ?></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
